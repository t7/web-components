...
export default function domset (id, props) {
	...
	: id in domset.ref
		...
		? domset.ref[id].cloneNode()
	// new nodes are referenced then cloned
	: (domset.ref[id] = document.createElement(id)).cloneNode();
...
domset.ref = ['circle', 'ellipse', 'defs', 'g', 'image', 'line', 'path', 'polygon', 'polyline', 'rect', 'svg', 'symbol', 'text', 'use'].reduce(
